...
class Game{
    private:
        Gameboard* gameboard;   // The game's board
        char colorToMove;       // The color of the player to move
        bool firstMove;         // Whether it is the first move of the game
    ...
    public:
        ...
        bool move(int oldFile, int oldRank, int newFile, int newRank) {
            ...
            if (successfulMove) {
                colorToMove = getOppositeColorToMove(); // Change the player-to-move
        ...
        void loadState() {
            ...
            if (prevMove != nullptr) {
                ...
                colorToMove = getOppositeColorToMove();
            } else colorToMove = 'W';
            firstMove = true;
        }
        ...
        // Destroys the game, going through past moves to destroy every piece
        ~Game() {
            // Delete pieces
            for (int i = 0; i < 8; i++) {
                for (int j = 0; j < 8; j++) {
